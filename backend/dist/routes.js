...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        ...
        const { userID } = jwt.verify(cookieToken, jwtSecret);
        cb(null, userID + ".jpg");
    }
});
const upload = multer({ storage });
const SnowConstructor = snowflakeId.default ?? snowflakeId;
...
router.route("/signUp").post(signVerify, doesExist, async (req, res, next) => {
    try {
...
router.route("/login").post(logVerify, async (req, res, next) => {
...
router.route("/users").get(isAuth, async (req, res, next) => {
    try {
...
router.route("/users/:userID").get(isAuth, async (req, res, next) => {
    try {
        const userID = req.params.userID;
        const [userInfo] = await pool.query(`SELECT userName , isAdmin , createdAt , pfp FROM users WHERE id = ?`, [userID]);
...
}).put(isAuth, upload.single("pfp"), (req, res, next) => {
    try {
        const userID = req.params.userID;
